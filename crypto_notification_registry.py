...
class NotificationRegistry:
    ...
    def update_notification(self, phone: str, volume_percentage: float, volume_time: str):
        ...
        if self.redis_client.exists(phone):
            ...
            log.warning(f"No existing notification preferences found for phone {phone}. Use ADD to create a new entry.")
    ...
    def delete_notification(self, phone: str):
        ...
        if self.redis_client.delete(phone):
            log.info(f"Deleted notificationpreferences for phone {phone}.")
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--phone", type=str, required=True, help="Phone number Example: +14691234567")
    parser.add_argument("--volume-percentage", type=float, help="Volume change percentage. Eg: 1 represents 1%")
    parser.add_argument("--volume-time", type=str, help="Volume time window (e.g., 1min, 5min, 1hr, 4hr)")
